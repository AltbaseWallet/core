...
struct ZanoSession {
  ...
  plain_wallet::hwallet wallet_id = 0;
  std::string address;
...
std::vector<std::filesystem::path> epic_wallet_backup_paths(const std::filesystem::path& work_dir) {
  ...
  paths.push_back(wallet_data / "wallet.seed");
...
bool epic_wallet_data_is_prepared(const std::filesystem::path& work_dir) {
  return safe_file_size(work_dir / "wallet_data" / "wallet.seed") > 0
...
std::optional<EpicWalletBackup> epic_wallet_backup(
  ...
  for (const auto& path : epic_wallet_backup_paths(work_dir)) {
    ...
    if (*key == "wallet_data/wallet.seed") has_seed = true;
...
std::string zano_wallet_rpc(
  plain_wallet::hwallet wallet_id,
  ...
  for (int attempt = 0; attempt < std::max(1, busy_attempts); ++attempt) {
    response = plain_wallet::invoke(wallet_id, rpc.str());
...
plain_wallet::hwallet zano_parse_wallet_id(const std::string& response) {
  if (json_has_error(response)) throw std::runtime_error(json_error_message(response));
  ...
  return static_cast<plain_wallet::hwallet>(std::stoull(wallet_id));
}

std::string zano_wallet_status(plain_wallet::hwallet wallet_id) {
  auto response = plain_wallet::sync_call("get_wallet_status", wallet_id, "{}");
  if (json_has_error(response)) throw std::runtime_error(json_error_message(response));
  return response;
}
...
bool zano_wallet_has_spendable_balance(plain_wallet::hwallet wallet_id, int busy_attempts = 1) {
  try {
...
ZanoBalance zano_wallet_balance(plain_wallet::hwallet wallet_id, int busy_attempts = 1) {
  ZanoBalance balance;
...
void ensure_zano_wallet_running(ZanoSession& session) {
  ...
  (void)plain_wallet::sync_call("run_wallet", session.wallet_id, "{}");
...
ZanoSession& ensure_zano_wallet(
  ...
  const auto init_response = plain_wallet::init(zano_node_host(params), zano_node_port(params), work_dir.string(), 0);
  ...
  if (wallet_exists) {
    open_response = plain_wallet::open(file_name, secret.engine_password);
  } else {
    const auto secret_derivation = zano_secret_derivation_from_mnemonic(mnemonic);
#ifdef __APPLE__
    (void)restore_height;
    open_response = plain_wallet::restore(file_name, secret.engine_password, secret_derivation, false, restore_from);
#else
    open_response = plain_wallet::restore(file_name, secret.engine_password, secret_derivation, false, restore_from, restore_height);
#endif
  }
...
std::optional<std::filesystem::path> epic_core_dll_path() {
#ifdef _WIN32
  const auto name = std::filesystem::path("altbase_epic_core.dll");
#elif defined(__APPLE__)
  const auto name = std::filesystem::path("libaltbase_epic_core.dylib");
#else
  const auto name = std::filesystem::path("libaltbase_epic_core.so");
#endif
...
std::string call_epic_core(const std::string& request) {
#ifdef _WIN32
  using RequestFn = char* (__cdecl*)(const char*);
  using FreeFn = void (__cdecl*)(char*);
#else
  ...
  std::call_once(load_once, [] {
    const auto path = epic_core_dll_path();
    if (!path.has_value()) {
      load_error = "Epic native core library was not found";
      return;
    }
#ifdef _WIN32
    module = LoadLibraryW(path->wstring().c_str());
    if (!module) {
      load_error = "failed to load Epic native core library";
      return;
    }
    request_fn = reinterpret_cast<RequestFn>(GetProcAddress(module, "altbase_epic_request"));
    free_fn = reinterpret_cast<FreeFn>(GetProcAddress(module, "altbase_epic_free"));
#else
    module = dlopen(path->string().c_str(), RTLD_NOW | RTLD_LOCAL);
    if (!module) {
      const char* error = dlerror();
      load_error = std::string("failed to load Epic native core library") + (error ? ": " + std::string(error) : "");
      return;
    }
    request_fn = reinterpret_cast<RequestFn>(dlsym(module, "altbase_epic_request"));
    free_fn = reinterpret_cast<FreeFn>(dlsym(module, "altbase_epic_free"));
#endif
    if (!request_fn || !free_fn) {
      load_error = "Epic native core exports are missing";
    }
  });

  if (!load_error.empty()) throw std::runtime_error(load_error);
  char* raw = request_fn(request.c_str());
  if (!raw) throw std::runtime_error("Epic native core returned null");
...
Bytes epic_derive_private_key(secp256k1_context* ctx, const Bytes& seed, const std::vector<uint32_t>& path) {
  ...
  if (secp256k1_ec_seckey_verify(ctx, private_key.data()) != 1) {
    throw std::runtime_error("invalid Epic master private key");
  }
  ...
  for (const auto index : path) {
    ...
    if (secp256k1_ec_seckey_verify(ctx, tweak.data()) != 1 ||
      ...
      throw std::runtime_error("invalid Epic child private key");
    }
...
std::string epicbox_address_from_mnemonic(const std::string& mnemonic) {
  ...
  if (secp256k1_ec_seckey_verify(ctx.get(), hashed_key.data()) != 1) {
    throw std::runtime_error("invalid Epic address private key");
  }
...
std::string zano_address_from_mnemonic(const std::string& mnemonic) {
  ...
  return account.get_public_address_str();
}
...
PrivacyLightWalletResult epic_wallet(const std::map<std::string, std::string>& params, const std::string& action) {
  ...
  if (mnemonic.empty()) {
    if (action != "snapshot") {
      auto result = not_ready("epic-unlocked-seed-required", "Epic address derivation requires the local seed to be unlocked.");
  ...
  const auto secret = privacy_wallet_secret({{"coin", "epic"}, {"mnemonic", mnemonic}});
  ...
  try {
    ...
    request << "{\"action\":\"" << json_escape(action) << "\","
            ...
            << "\"dataDir\":\"" << json_escape(epic_dir.string()) << "\","
    ...
    const auto response = call_epic_core(request.str());
...
PrivacyLightWalletResult zano_wallet(const std::map<std::string, std::string>& params, const std::string& action) {
  ...
  if (mnemonic.empty()) {
    if (action != "snapshot") {
      auto result = not_ready("zano-unlocked-seed-required", "Zano address derivation requires the local seed to be unlocked.");
  ...
  const auto secret = privacy_wallet_secret({{"coin", "zano"}, {"mnemonic", mnemonic}});
  const auto address = zano_address_from_mnemonic(mnemonic);
  ...
  if (action == "warm") {
    ...
    try {
      ...
      try {
        set_zano_wallet_height(result, zano_wallet_status(session.wallet_id));
      } catch (...) {
      }
      uint64_t unlocked = 0;
  ...
  try {
    ...
    if (session.initial_sync_pending) {
      ...
      result.transactions = zano_wallet_rpc(
        ...
        "{\"offset\":0,\"count\":50,\"update_provision_info\":true,\"exclude_mining_txs\":false,\"exclude_unconfirmed\":false,\"order\":\"FROM_END_TO_BEGIN\"}",
...
PrivacyLightWalletResult privacy_light_wallet(const std::map<std::string, std::string>& params) {
  ...
  if ((action == "ensure" || action == "warm" || action == "send") && mnemonic.empty()) {
    return not_ready("missing-mnemonic", "Mnemonic is required for native privacy light wallet");
  }

  if (!mnemonic.empty()) {
    (void)privacy_wallet_secret({{"coin", coin}, {"mnemonic", mnemonic}});
  }
