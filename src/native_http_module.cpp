...
HttpResponse do_request(
  ...
  // Keep DNS, TLS sessions and reusable connections for this worker thread.
  // Reset request options so a later GET cannot inherit a POST body or headers.
  thread_local std::unique_ptr<CURL, decltype(&curl_easy_cleanup)> connection(curl_easy_init(), curl_easy_cleanup);
